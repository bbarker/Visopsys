			   ...
			   void *argv[])
{
  ...
  // by Davide Airaghi
  if (kernelCurrentProcess->ring0)
    extra_priv = PRIVILEGE_KERNEL;
  ...
  processId = createNewProcess(name, kernelCurrentProcess->priority,
			       kernelCurrentProcess->privilege | extra_priv, // modified by Davide Airaghi
			       &execImage, 0);
  if (processId < 0)
  ...
  // cast required! Davide Airaghi
  ((kernelTSS *)(newProcess->taskStateSegment))->ESP -= 4;
...
// check if the given process can use I/O ports specified
// return: 0 = allowed, -1 = not allowed , < -1 = error!
// Davide Airaghi
int kernelMultitaskerGetIOperm(int processId,unsigned int portNum) {
  int status = 0;
  kernelProcess *Process = NULL;
  ...
  if (Process == NULL)
    {
      // There's no such process
      kernelError(kernel_error, "No process %d to get IOPerm", processId);
      return (status = ERR_NOSUCHPROCESS);
    }
    ...
    if (portNum > Process->max_io_port)
	return -1;	
	
    status = GET_PORT_BIT(portNum, Process->IOMap)
    
    if (status == PORT_VAL_BIT_TRUE)
	return (status = 0);
    else
	return (status = -1);    
}

// Allow the specified process to use the given IO port 
// Davide Airaghi
int kernelMultitaskerAllowIO(int processId, unsigned int portNum) {
  if (!(kernelCurrentProcess->privilege == PRIVILEGE_SUPERVISOR))
    {
      kernelError(kernel_error, "An unprivileged process cannot set IO perms");
      return (ERR_PERMISSION);
    }
 return kernelMultitaskerSetIOperm(processId,portNum,PORT_VAL_BIT_TRUE);
}

// Don't allow the specified process to use the given IO port 
// Davide Airaghi
int kernelMultitaskerNotAllowIO(int processId, unsigned int portNum) {
  if (!(kernelCurrentProcess->privilege == PRIVILEGE_SUPERVISOR))
    {
      kernelError(kernel_error, "An unprivileged process cannot set IO perms");
      return (ERR_PERMISSION);
    }
 return kernelMultitaskerSetIOperm(processId,portNum,PORT_VAL_BIT_FALSE);
}

// added by Davide Airaghi
// return 0 if ok, < 0 else
int kernelMultitaskerIsLowLevelProcess(int pId) {
    kernelProcess * thisProcess;
    thisProcess = getProcessById(pId);
    if (thisProcess == NULL)
	return (ERR_NOSUCHPROCESS);
    if (thisProcess->ring0)
	return 0;
    else
	return -1;
}
