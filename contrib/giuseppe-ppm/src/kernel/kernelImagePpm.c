...
static void skipWhiteSpace(unsigned char *p, int *pos,
	int maxLen)
{
	int exit_loop = 0;
	
	while((*pos) < maxLen && !exit_loop)
	{
		if( (p[*pos]) == '#' )
		{
...
static int getNextValue(unsigned char *p, int *pos,
	int maxLen)
{
	int l = 0;
	char buf[16];
...
static unsigned char getByte(unsigned char *p, int *pos,
	int maxLen)
{
	if((*pos) < maxLen)
		return p[(*pos)++];
...
static unsigned short getShort(unsigned char *p, int *pos,
	int maxLen)
{
	unsigned short r = 0;
	...
	int reqHeight __attribute__((unused)), image *loadImage)
{
	...
	int pos = 0;
	int binary = (imageFileData[1] == '6'); // If magic is 'P6', this is a binary PPM
	double factor;
	...
	if(width == 0 || height == 0 || maxCompValue == 0
		|| maxCompValue > 0xFFFF)
	...
	skipWhiteSpace(imageFileData, &pos, dataLength);
...
static int save(const char *fileName, image *saveImage)
{
	...
	for(x = 0, px = (saveImage->width * saveImage->height), savePixels = saveImage->data;
		x < px; x++)
	{
