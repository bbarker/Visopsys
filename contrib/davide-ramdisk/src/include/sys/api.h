...
// next four added by Davide Airaghi
#define _fnum_diskRamDiskCreate			     2017
#define _fnum_diskRamDiskDestroy		     2018
#define _fnum_diskRamDiskInfo			     2019
...
// next funtion created by Davide Airaghi
_X_ static inline int diskRamDiskCreate(int diskNum, unsigned size)
{
  // Proto: int kernelRamDiskCreate(int, unsigned);
  // Desc : This function attempts to create 'size' bytes of fake-disk-data for RamDisk ram'disknum'.
  return (sysCall_2(_fnum_diskRamDiskCreate, (void *) diskNum, (void *) size));
}

// next funtion created by Davide Airaghi
_X_ static inline int diskRamDiskDestroy(int diskNum)
{
  // Proto: int kernelRamDiskDestroy(int);
  // Desc : This function attempts to destroy fake-disk-data for RamDisk ram'disknum'.
  return (sysCall_1(_fnum_diskRamDiskDestroy, (void *) diskNum));
}

// next funtion created by Davide Airaghi
_X_ static inline int diskRamDiskInfo(int diskNum, ramdisk * rdinfo)
{
  // Proto: void kernelRamDiskInfo(int);
  // Desc : This function attempts to put into 'rdinfo' info about RamDisk ram'disknum'.
  return (sysCall_2(_fnum_diskRamDiskInfo, (void *) diskNum, (void*) rdinfo));
}
...
// modified by Davide Airaghi to add "options" parameter
_X_ static inline int filesystemMount(const char *name, const char *mp, const char *options)
{
  // Proto: int kernelFilesystemMount(const char *, const char *, const char * options)
  // Desc : Mount the filesystem on disk 'name', using the mount point specified by the absolute pathname 'mp' and the options specified in 'options'.  Note that no file or directory called 'mp' should exist, as the mount function will expect to be able to create it.
  return (sysCall_3(_fnum_filesystemMount, (void *) name, (void *) mp, (void *)options));
}
